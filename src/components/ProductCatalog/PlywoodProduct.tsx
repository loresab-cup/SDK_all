...
const PlywoodProduct: React.FC<Props> = ({ title }) => {
    ...
    const [selectedPrice1, setSelectedPrice1] = useState<{ row: number; col: number } | null>(null);
    const [selectedPrice2, setSelectedPrice2] = useState<{ row: number; col: number } | null>(null);
    ...
    const handlePrice1Click = (rowIdx: number, colIdx: number, price: string) => {
        setSelectedPrice1({ row: rowIdx, col: colIdx });
        console.log('Фанера (тонкая):', price);
    };

    const handlePrice2Click = (rowIdx: number, colIdx: number, price: string) => {
        setSelectedPrice2({ row: rowIdx, col: colIdx });
        console.log('Фанера (толстая):', price);
    };
                                        ...
                                        {priceRows1.map((row, rowIdx) => (
                                            <tr key={rowIdx}>
                                                ...
                                                {row.prices.map((price, colIdx) => {
                                                    const isSelected = selectedPrice1?.row === rowIdx && selectedPrice1?.col === colIdx;
                                                    return (
                                                        <td
                                                            key={colIdx}
                                    ...
                                    <tbody>
                                        {priceRows2.map((row, rowIdx) => (
                                            <tr key={rowIdx}>
                                                ...
                                                <td>{row.surface}</td>
                                                {row.prices.map((price, colIdx) => {
                                                    const isSelected = selectedPrice2?.row === rowIdx && selectedPrice2?.col === colIdx;
                                                    return (
                                                        <td
                                                            key={colIdx}
